...
resource "aws_eks_node_group" "petclinic_eks_nodegrp" {
  ...
  subnet_ids      = var.subnets
  instance_types  = ["t3a.small"] #e.g. ["t3a.small"]
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.pet-AmazonEKSFargatePodExecutionRolePolicy
  ]
}
...
resource "aws_iam_role_policy_attachment" "petnode-AmazonEKSFargatePodExecutionRolePolicy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSFargatePodExecutionRolePolicy"
  role       = aws_iam_role.pet_role_node.name
}